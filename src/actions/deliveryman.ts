...
export async function addDeliveryman(data: Deliveryman) {
  try {
    ...
    const docRef = await addDoc(deliverymanRef, {
      firstName,
      lastName,
      email,
      phone,
...
// READ: Get deliverymen by status
export async function getDeliverymenByStatus(
  status: "active" | "inactive" | "suspended",
) {
  try {
    const deliverymanRef = collection(db, "deliverymen");
    const q = query(deliverymanRef, where("status", "==", status));
    ...
    console.error("Error getting deliverymen by status:", error);
    return { success: false, error: (error as Error).message };
  }
}

// READ: Get deliverymen by filter (any field)
...
export async function updateDeliveryman(
  ...
  try {
    ...
    await updateDoc(deliverymanRef, updatedData);
...
// UPDATE: Update deliveryman status
...
// UPDATE: Approve a deliveryman
// export async function approveDeliveryman(id: string) {
//   try {
//     const deliverymanRef = doc(db, "deliverymen_applications", id);
...
//     return { success: true };
//   } catch (error) {
//     console.error("Error approving deliveryman:", error);
//     return { success: false, error: (error as Error).message };
//   }
// }

// UPDATE: Suspend a deliveryman
export async function suspendDeliveryman(id: string) {
  try {
    ...
    console.error("Error suspending deliveryman:", error);
    return { success: false, error: (error as Error).message };
  }
}

// UPDATE: Reactivate a suspended deliveryman
export async function reactivateDeliveryman(id: string) {
  try {
    ...
    console.error("Error reactivating deliveryman:", error);
    return { success: false, error: (error as Error).message };
  }
}
// /** @format */
